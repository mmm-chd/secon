...
# include_in_schema -> used to hidden the routes from the documentation
...
def put_post(post_id: int, post_data: PostCreate, db: Annotated[Session, Depends(get_db)]):
    result = db.execute(select(models.Post).where(models.Post.id == post_id))
    post = result.scalars().first()
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Post not found")
...
def delete_post_by_id(post_id: int, db: Annotated[Session, Depends(get_db)]):
    ...
    if post:
        db.delete(post)
        db.commit()
    else: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Post not found")
